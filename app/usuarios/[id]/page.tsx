...
export default async function DetalleUsuarioPage({
  ...
  const accionBanear = banearUsuario.bind(null, usuario.id);
  const accionDesbanear = desbanearUsuario.bind(null, usuario.id);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {usuario.estado === "Activo" ? (
            <form action={accionBanear}>
              <Button type="submit" variant="destructive">
                <Ban className="mr-2 h-4 w-4" /> Suspender Cuenta
              </Button>
            </form>
          ) : (
            <form action={accionDesbanear}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* COLUMNA 1: Info General y Moderación (Feedback App) */}
